...
if (navigator.geolocation) {
	...
	function getCurrentLocation (position) {
		...
		$.getJSON("http://api.openweathermap.org/data/2.5/weather?lat=" + latitude + "&lon=" + longitude + "&APPID=c7d6dace33284384aae2d473db6dc94b", function (data) {
	...
	$("form").on("submit", function(event) {
		...
		$.getJSON("http://api.openweathermap.org/data/2.5/weather?q=" + city + "&APPID=c7d6dace33284384aae2d473db6dc94b", function (data) {
			var rawJson = JSON.stringify(data);
			var json = JSON.parse(rawJson);
			updateWeather(json); //Update Weather parameters
		});
